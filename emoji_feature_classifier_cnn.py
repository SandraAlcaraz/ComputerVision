...
if __name__ == "__main__":
    ...
    if args.retrain or not 'filter_model.h5' in os.listdir():
        ...
        # history = model.fit(X_filter_train, Y_filter_cat_train, batch_size=batch_size, epochs=epochs, verbose=1, 
        #                validation_data=(X_filter_test, Y_filter_cat_test))
        # Fit the model on the batches generated by datagen.flow().
        history = model.fit_generator(datagen.flow(X_filter_train, Y_filter_cat_train, batch_size=batch_size),
                                    steps_per_epoch=int(np.ceil(X_filter_train.shape[0] / float(batch_size))),
                                    epochs=epochs,
                                    validation_data=(X_filter_test, Y_filter_cat_test),
                                    workers=4)
        print('Got history')
        
        model.evaluate(X_filter_test, Y_filter_cat_test)
